...
void uart1_init() {
	...
	MCF_INTC0_ICR14 |= MCF_INTC_ICR_IL(0x04);
...
void uart1_download() {
	uint8_t g_program_mode = 1;
	int data = 0;
	uint32_t bytes_read = 0;
	uint8_t map[64] = {0};
	int i = 0;
	
	while(g_program_mode == 1) {
		// New byte received
		while((MCF_UART1_USR & MCF_UART_USR_RXRDY) || (MCF_UART1_USR & MCF_UART_USR_FFULL)) {
			data = MCF_UART1_URB;
			map[i] = data;
			i++;
			uart1_write(0x05);
	...
	printf("Num of bytes read: %i\n", bytes_read);
	
	// Convert mangled values to map values 
	/*
	 * 240 = 0x00 empty
	 * 255 = 0xFF wall
	 * 244 = 0x60 ghost
